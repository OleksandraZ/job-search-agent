...
def test_fetches_from_known_source_with_search_terms_injected(monkeypatch):
    ...
    monkeypatch.setitem(registry.ADAPTERS, "fake_adapter", fake_fetch)
...
def test_one_source_failing_does_not_stop_the_others(monkeypatch):
    ...
    monkeypatch.setitem(registry.ADAPTERS, "failing_adapter", failing_fetch)
    monkeypatch.setitem(registry.ADAPTERS, "working_adapter", working_fetch)
